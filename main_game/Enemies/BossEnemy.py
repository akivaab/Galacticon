...
class BossEnemy(Enemy):
    def __init__(self, num_hits, x=320, y=10, boss_img=None, ship_speed=2.5, bullet_speed=4, fire_freq=225):
        if boss_img is None:
            boss_img = pygame.image.load("assets/enemies/boss.png").convert()
            boss_img.set_colorkey((0, 0, 0))
        super().__init__(x, y, boss_img, ship_speed, bullet_speed, fire_freq)
        self.direction = 1
        ...
        self.bullet_image = pygame.transform.scale(bullet_img, (52, 52))
    ...
    def random_fire(self):
        if random.randint(0, self.fire_freq) == 42:
            self.bullets_fired.append(Bullet(self.x - 14, self.y + 32, self.bullet_image, self.bullet_speed))
            self.bullets_fired.append(Bullet(self.x + 5, self.y + 32, self.bullet_image, self.bullet_speed))
            self.bullets_fired.append(Bullet(self.x + 24, self.y + 32, self.bullet_image, self.bullet_speed))
